...
def run(comm, env, policy, policy_path, action_bound, optimizer):
    ...
    for id in range(MAX_EPISODES):
        ...
        while not group_terminal :

            # print(env.index,"before gather")

            state_list = comm.gather(state, root=0)

            # print(env.index,"after gather")
        ...
        if env.index == 0:
            if global_update != 0 and global_update % 20 == 0:
                torch.save(policy.state_dict(), policy_path + '/stage3_{}.pth'.format(global_update))
...
if __name__ == '__main__':
    ...
    env = StageWorld(360, index=rank, num_env=NUM_ENV)
    reward = None
